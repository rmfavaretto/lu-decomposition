...
class Main {
	public static String path = "/home/simon/projects/metodi_calcolo_scientifico/LuEig/matrice/";
	...
	public static void main(String[] argv) {
		 System.out.println("TEST        \tERRORE RELATIVO \tERRORE PRIMA COMP.\tAUTOVALORE NUMERO 7\tTIME TO SOLVE\tTIME TO EIGEN");
		 System.out.println("------------\t----------------\t------------------\t-------------------\t-------------\t-------------");
		 
		 String[] names = {
				 "easy-10", "easy-100", "easy-1000", 
				 "bad-10", "bad-100", "bad-500", "bad-1000", 
				 "verybad-10", "verybad-100", "verybad-500", "verybad-1000",
				 "rand-10", "rand-100", "rand-1000", "rand-5000",
				 "eig-10", "eig-20", "eig-30", "eig-40", "eig-50", "eig-100", "eig-1000", "eig-2000", "eig-5000"
		 };
		 ...
		 for (int i = 0; i < names.length; i++) 
		 {
			   ...
			   String printName = nameFile.replace("matrice-", "").replace("-symm", "").replace(".dat", "");
			   
			   if(nameFile.startsWith("matrice-eig")) {
				   System.out.printf("%12s\t%16s\t%18s\t%19s\t%12s\t\t%2dms%n", printName, erroreRelativo, errorePrimaComp, settimoAutovalore, "-", (fineE-inizioE));
			   } else if(nameFile.startsWith("matrice-rand")) {
				   System.out.printf("%12s\t%16s\t%18s\t%19s\t%2dms\t\t%12s%n", printName, erroreRelativo, errorePrimaComp, settimoAutovalore, (fineS-inizioS), "-");
			   } else {
				   System.out.printf("%12s\t%16s\t%18s\t%19s\t%2dms\t\t%2dms%n", printName, erroreRelativo, errorePrimaComp, settimoAutovalore, (fineS-inizioS), (fineE-inizioE));
			   }
		 }
	}
}